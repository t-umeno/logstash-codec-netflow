...
def iana2hash(url)
  ...
  CSV.new(open(url), :headers => :first_row, :converters => :numeric).each do |line|
    ...
    template_fields[0][line['ElementID']] = [iana2bindata(line['Data Type']).to_sym]
    if template_fields[0][line['ElementID']][0] != :skip
      template_fields[0][line['ElementID']] << line['Name'].to_sym
    end
  end

  # Overrides
  template_fields[0][210][0] = :skip # 210 is PaddingOctets so skip them properly
  template_fields[0][210].delete_at(1)

  # Generate the reverse PEN (PEN 29305)
  reversed = template_fields[0].reject { |k|
  ...
  template_fields[29305] = Hash[reversed]
